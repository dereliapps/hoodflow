...
const v4Tickers = ["INTC", "COIN", "PLTR", "USAR"] as const;
const v3Tickers = ["SLV", "CUSO"] as const;
if (
  Object.keys(infrastructure.v3VerifiedAssets).length !== v3Tickers.length
  || v3Tickers.some((ticker) => !(ticker in infrastructure.v3VerifiedAssets))
) {
  throw new Error("The direct V3 fork fixtures do not match the configured V3 route registry");
}
const amountIn = parseUnits("1", 6);
const fundingAmount = amountIn * BigInt(v4Tickers.length + v3Tickers.length);
...
async function executeV4RoundTrip(ticker: typeof v4Tickers[number]) {
  ...
  for (const route of V4_POOL_CANDIDATES) {
    try {
      const result = await quoter.quoteExactInputSingle.staticCall(
        buildQuoteParams(tokenAddress, amountIn, route),
  ...
  if (quotes.length === 0) throw new Error(`${ticker}/USDG returned no live V4 quote`);
  ...
  const permit: PermitSingle = {
    details: {
      ...
      nonce: BigInt(currentPermit.nonce ?? currentPermit[2] ?? 0n),
    },
  ...
  await router.execute.staticCall("0x0a", [calldata.inputs[0]], latest.timestamp + 300);

  const before = BigInt(await outputToken.balanceOf(buyer.address));
  ...
  const received = BigInt(await outputToken.balanceOf(buyer.address)) - before;
  if (!receipt || receipt.status !== 1 || received < minAmountOut) {
    throw new Error(`Direct ${ticker} buy did not satisfy the protected minimum output`);
  }
  ...
  if (erc20Allowance !== 0n || permitAmountAfter !== 0n) {
    throw new Error(`${ticker} exact direct-buy permissions were not fully consumed`);
  }
  ...
  for (const route of V4_POOL_CANDIDATES) {
    try {
      const result = await quoter.quoteExactInputSingle.staticCall(
        buildExactInputQuoteParams(tokenAddress, USDG_ADDRESS, received, route),
      ) as readonly [bigint, bigint];
      const amountOut = BigInt(result[0]);
  ...
  if (sellQuotes.length === 0) throw new Error(`${ticker}/USDG returned no live V4 sell quote`);
  ...
  await (await outputWithSigner.approve(PERMIT2_ADDRESS, received)).wait();
  const sellPermitState = await permit2.allowance(
    buyer.address,
    tokenAddress,
    UNIVERSAL_ROUTER_ADDRESS,
  ) as { nonce?: bigint; 2?: bigint };
  ...
  const sellPermit: PermitSingle = {
    details: {
      token: tokenAddress,
      amount: received,
      expiration: sellBlock.timestamp + 600,
      nonce: BigInt(sellPermitState.nonce ?? sellPermitState[2] ?? 0n),
    },
    spender: UNIVERSAL_ROUTER_ADDRESS,
  ...
  const sellTransaction = await router.execute(
    sellCalldata.commands,
    sellCalldata.inputs,
    sellBlock.timestamp + 300,
  );
  const sellReceipt = await sellTransaction.wait();
  const sellReceived = BigInt(await usdG.balanceOf(buyer.address)) - usdGBeforeSell;
  const sellTokenAllowanceAfter = BigInt(await outputWithSigner.allowance(buyer.address, PERMIT2_ADDRESS));
  const sellRouterPermit = await permit2.allowance(
    buyer.address,
    tokenAddress,
    UNIVERSAL_ROUTER_ADDRESS,
  ) as { amount?: bigint; 0?: bigint };
  const sellRouterAllowanceAfter = BigInt(sellRouterPermit.amount ?? sellRouterPermit[0] ?? 0n);
  if (!sellReceipt || sellReceipt.status !== 1 || sellReceived < minSellOut) {
    throw new Error(`Direct ${ticker} sell did not satisfy the protected minimum USDG output`);
  }
  if (sellTokenAllowanceAfter !== 0n || sellRouterAllowanceAfter !== 0n) {
    throw new Error(`${ticker} exact direct-sell permissions were not fully consumed`);
  }

  console.log(`direct V4 route passed: ${ticker}`);
  return {
    pair: `${ticker}/USDG`,
    buy: {
...
async function executeV3Buy(ticker: typeof v3Tickers[number]) {
  const tokenAddress = ROBINHOOD_TOKENS[ticker];
...
const v4Results = [];
for (const ticker of v4Tickers) {
  v4Results.push(await executeV4RoundTrip(ticker));
}
const v3Results = [];
for (const ticker of v3Tickers) {
  v3Results.push(await executeV3Buy(ticker));
}
