    ...
    <section className="market-board">
      ...
      <div className="token-market-table">
        ...
        {!marketsLoading && visibleMarkets.map((market, index) => <article className="token-market-row" key={market.address} role="button" tabIndex={0} aria-label={`Open ${market.symbol} market`} onClick={() => inspectMarket(market)} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") inspectMarket(market); }}>
          ...
          <div className="token-metric"><strong>{compactMoney(market.priceUsd, true)}</strong><small>{compact(market.address)}</small></div>
          <strong className={`market-change ${(market.priceChange24h ?? 0) >= 0 ? "up" : "down"}`}>{percent(market.priceChange24h)}</strong>
          <div className="token-metric"><strong>{compactMoney(market.volume24h)}</strong><small>{market.transactions24h ? `${market.transactions24h.toLocaleString("en-US")} trades` : "24 hours"}</small></div>
          <div className="token-metric"><strong>{compactMoney(market.liquidityUsd)}</strong><small>Pool liquidity</small></div>
          <div className="token-metric"><strong>{compactMoney(market.marketCapUsd ?? market.fdvUsd)}</strong><small>{market.marketCapUsd !== null ? "Market cap" : market.fdvUsd !== null ? "FDV" : market.fdvInVirtual !== null ? `${market.fdvInVirtual.toLocaleString("en-US", { maximumFractionDigits: 0 })} VIRTUAL FDV` : "Not reported"}</small></div>
