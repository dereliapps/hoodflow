...
export default function ReferralRewards({ walletAddress, walletProvider, onWallet, notify }: Props) {
  ...
  return <section className="page inner-page rewards-page">
    <div className="rewards-hero"><div><p className="eyebrow">HOODFLOW REWARDS / SEASON 0</p><h1>Invite wallets.<br /><span>Earn after verified trades.</span></h1><p>Create one referral link and earn points only after an invited wallet completes its first eligible HoodFlow trade. Clicks alone earn nothing.</p></div><div className="points-orbit"><span>YOUR BALANCE</span><strong>{data.profile?.points.toLocaleString("en-US") ?? "—"}</strong><small>HF POINTS</small></div></div>
