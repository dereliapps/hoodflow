...
function v3Trade(tokenOut = stock, amountIn = 1_000_000n, minimumHopPrices: readonly bigint[] = []) {
  ...
  const swap = coder.encode(
    ...
    [wallet, amountIn, 10n, path, true, minimumHopPrices],
  );
...
test("accepts a value-bounded reviewed Stock Token settlement", () => {
  ...
  assert.throws(
    () => verifyEligibleReferralTrade({ ...v3Trade(stock, 1_000_000n, [1n]), wallet }),
    /unsupported V3 hop guards/,
  );
});
