...
function decodeV3(input: string, wallet: string): SwapShape {
  const [
    ...
    pathValue,
    payerIsUserValue,
    minimumHopPricesValue,
  ] = CODER.decode(
    ["address", "uint256", "uint256", "bytes", "bool", "uint256[]"],
    input,
  );
  const path = String(pathValue).toLowerCase();
  const minimumHopPrices = Array.from(minimumHopPricesValue as Result, (value) => BigInt(value));
  if (!/^0x[a-f0-9]{86}$/.test(path)) throw new Error("The referral trade uses an unsupported V3 path.");
  if (minimumHopPrices.length !== 0) throw new Error("The referral trade uses unsupported V3 hop guards.");
